...
begin
  ...
  repeat
    ...
    for m:=0 to k-1 do begin
      i := m;
      while i<nx do begin  //for i=m to nx step istep
        ct := CMult(cw,cx[i+k]);
...
begin
  ...
  SetLength(tmp,len);
  for i:=0 to len-1 do begin //for each col
...
procedure Normalize(cArr:TComplexDynArray; buf:TByteDynArray; mode:TProcessMode); //1D
var
  j,len:integer;
  m,n,r:Double;
begin
  ...
  for j:=0 to len-1 do begin//for j:=1 to len do begin
    case mode of
      module: r := Hypot(cArr[j].Real,cArr[j].Imag);
      phase:  r := ArcTan2(cArr[j].Imag,cArr[j].Real);
      real:   r := cArr[j].Real;
    else      r := cArr[j].Imag;
    end;
    if m<r then m:=r;
...
procedure Normalize(cArr:TComplex2DArray; buf:TByte2DArray; mode:TProcessMode);  //2D
var
  i,j,len:integer;
  m,n,r:Double;
begin
  ...
  for i:=0 to len-1 do
    for j:=0 to len-1 do begin
      case mode of
        module: r := Hypot(cArr[i,j].Real,cArr[i,j].Imag);
        phase:  r := ArcTan2(cArr[i,j].Imag,cArr[i,j].Real);
        real:   r := cArr[i,j].Real;
      else      r := cArr[i,j].Imag;
      end;
      if m<r then m:=r;
      if n>r then n:=r;
      cArr[i,j].Real := r;
